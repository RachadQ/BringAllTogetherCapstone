Shader "Custom/PhongSpecular"
{
    ...
    SubShader
    {
	   ...
	   #pragma surface surf Phong
		...
		// Create own view direction shader
		fixed4 LightingPhong(SurfaceOutput s, fixed3 lightDir, half3
			viewDir, fixed atten)
		{
			// Reflection 
			//dot product the surface normal and the direction light sdirection
			//get the directon
			float NdotL = dot(s.Normal, lightDir);

			//get reflection from normalize vector
			//get the direction itss going
			float3 reflectionVector = normalize(2.0 * s.Normal * NdotL - lightDir);

			// Specular 
			//get the shinest of the object
